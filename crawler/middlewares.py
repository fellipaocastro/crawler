...
# This middleware can be used to avoid re-visiting already visited items, which
# can be useful for speeding up the scraping for projects with immutable items,
        ...
        for x in result:
            ...
            if isinstance(x, Request):
                ...
                if visit_id:
                    ...
                    x['visit_status'] = 'new'
            if visited:
                ret.append(EcpItem(visit_id=visit_id, visit_status='old'))
            else:
                ret.append(x)
        return ret

    def _visited_id(self, request):
        return request.meta.get(self.VISITED_ID) or \
